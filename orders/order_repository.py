...
class OrderRepository:
    ...
    def find_users_orders(self, user: User) -> List[Order]:
        return self._model_manager().get_queryset().select_related('buyer', 'supplier').involving(user=user)

    def find_all_orders(self) -> List[Order]:
        return self._model_manager().get_queryset().select_related('buyer', 'supplier').all()
