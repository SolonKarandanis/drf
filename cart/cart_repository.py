...
class CartRepository:
    ...
    def _model_manager(self) -> CartManager:
        return Cart.objects

    def fetch_user_cart(self, logged_in_user: User) -> Cart:
        cart = self._model_manager().get_queryset() \
            .with_cart_items() \
    ...
    def fetch_user_cart_with_products_and_users(self, logged_in_user: User) -> Cart:
        ...
        cart = self._model_manager().prefetch_related(cart_items_products_prefetch)\
            .get(user=logged_in_user)
        return cart

    def update_cart(self, cart: Cart) -> Cart:
        cart = self._model_manager().update_cart(cart)
        return cart
